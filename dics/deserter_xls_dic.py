...
PATTERN_RTZK: Final = (
    r"(?i)(?:Призваний[:,]?\s*)?(?:(?:\d{2}\.\d{2}\.(?:\d{4}|\d{2}))\s*(?:року|[,.:;\s*])?\s*)?((?:\d+\s+відділ(?:ом)?\s+)?[А-ЯҐЄІЇ][а-яґєії\']+(?:[-\s][А-ЯҐЄІЇ][а-яґєії\']+)*\s+(?:[ОМР]{0,2}ТЦК|РВК|ОВК|РЦТК)(?:\s+та\s+СП)?(?:[\s,]+(?:(?:в\s+)?(?:м\.?|місто|н\.п\.?)\s*)?[А-ЯҐЄІЇ][а-яґєії\']+(?:[-\s][А-ЯҐЄІЇ][а-яґєії\']+)*)*(?:[\s,]+(?:област[ьі]|обл))?)"
)
