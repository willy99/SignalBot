...
PATTERN_DESERTION_PLACE_MAPPING : Final[Dict[str, str]] = {
    r'(?i)(з|самовільн[(е|ого)\s]*\sзалиш[(ення|ив)\s]*)\s*район[у\s*]+виконання\s*(?:бойового\s*)?(завдан[(ня|ь)\s]*)(за\s+призначенням)?': 'РВБЗ',
    r'(?i)після\s+(проходження|огляду)\s+(військово-лікарської\s+комісії|влк)': 'лікування',
    r'(?i)(курсанта\s+.*?навчальної\s+роти|(після|з)\s+.*?БЗВП)': 'НЦ',
...
PATTERN_RTZK: Final = (
    r"(?i)(?:Призваний[:,]?\s*)?(?:(?:\d{2}\.\d{2}\.(?:\d{4}|\d{2}))\s*(?:року|[,.:;\s*])?\s*)?((?:\d+(?:\s?-\s?[а-яіїє]*)?\s?відділ(?:ом)?\s*)?[А-ЯҐЄІЇ][а-яґєії\'ʼ’]+(?:[-\s][А-ЯҐЄІЇ][а-яґєії\'ʼ’]+)*\s+(?:[ОМР]{0,2}ТЦК|РВК|ОМВК|ОВК|РЦТК)(?:\s+та\s+СП)?(?:[\s,]+(?:(?:в\s+)?(?:м\.?|місто|н\.п\.?)\s*)?[А-ЯҐЄІЇ][а-яґєії\'ʼ’]+(?:[-\s][А-ЯҐЄІЇ][а-яґєії\'ʼ’]+)*)*(?:[\s,]+(?:област[ьі]|обл))?)"
)
...
PATTERN_LOCALITY: Final[str] = r'(?:пункті|н\.п\.|\sс[елаоу\.]*)\s?((?:[А-ЯҐЄІЇ][а-яґєії]+(?:[\s-][А-ЯҐЄІЇ][а-яґєії]+)*?))(?=\s+[А-ЯҐЄІЇ][а-яґєії]+(?:-[А-ЯҐЄІЇ][а-яґєії]+)*\s+обл[асті\.]*)'
...
PATTERN_PIECE_3_END : Final[str ]= r'(?:4[\.\s\)]*)?Хто\s+проводить\s+службове\s+розслід[а-яіїє]*[\s,]*строки\s+його\s+проведе[нп]+я'

PATTERN_PIECE_4_START : Final[str] = r'Хто проводить службове (?:розслід[а-яіїє]*)[,\.\s]*строки його[,\.\s]*проведе[нп]*я\.'
PATTERN_PIECE_4_END : Final[str] = r'(?:5[\.\s\)]*)?Військове\s+звання.*?безпосередніх\s+начальників'
