...
PATTERN_DESERTION_PLACE_MAPPING : Final[Dict[str, str]] = {
    ...
    r'(?i)(курсанта\s+.*?навчальної\s+роти|граніт|(після|з)\s+.*?БЗВП)': 'НЦ',
...
PATTERN_ADDRESS_CONTENT: Final = r'^((?:(?!Мати:|РНОКПП|Батько:|Дружина:|Родичі:|Близькі|тел\.|;|\n).)+)'
...
PATTERN_LOCALITY: Final[str] = r'(?:пункт[і|у]|н\.?п\.?|міста|м.|\sс[елаоу\.]*)\s?((?:[А-ЯҐЄІЇ][а-яґєії\'ʼ’]+(?:[\s-][А-ЯҐЄІЇ][а-яґєії\'ʼ’]+)*?))[,.]?(?=\s+[А-ЯҐЄІЇ][а-яґєії]+(?:-[А-ЯҐЄІЇ][а-яґєії]+)*\s+обл[асті\.]*)'
...
PATTERN_PIECE_3_START : Final[str] = r'(?:Стислі демографіч[н|и][і|ї] да[н|п]і|Адреса проживання.*?Близькі родичі|Близькі родичі.*?Адреса проживання|призваний на військову службу, освіта, сімейний стан)'
PATTERN_PIECE_3_END : Final[str ]= r'(?:4[\.\s\)]*)?Хто\s+проводить\s+службове\s+розслід[а-яіїє]*[\s,]*строки\s+його\s+проведе[нп]+я'

PATTERN_PIECE_4_START : Final[str] = r'Хто проводить службове (?:розслід[а-яіїє]*)[,\.\s]*строки його[,\.\s]*проведе[нип]*я\.'

PATTERN_PIECE_4_END : Final[str] = r'(?:5[\.\s\)]*)?Військове\s+звання.*?безпосередніх\s+[нип]*ачальників'

PATTERN_ERDR_CONDITIONS_START:Final[str] = r'Короткий виклад обставин|Терміново|ВИМОГА|Командиру військової частини А0224|Фабула'
PATTERN_ERDR_CONDITIONS_END:Final[str] = r'Найменування та код ЄДР|З повагою'
PATTERN_ERDR_CONDITIONS_NAME = r"(?:(?:[А-ЩЬЮЯҐЄІЇ][А-ЯІЇЄҐа-яіїєґ\'ʼ’-]{3,}\s?){1,2})\s+(?:[А-ЯҐЄІЇ][а-яґєії\'ʼ’]{1,}(?:-[А-ЯҐЄІЇ][а-яґєії\'ʼ’]{1,})?)\s+(?:(?:[А-ЯҐЄІЇ][а-яґєії\-\'ʼ’]{2,}\s?){1,2})"
