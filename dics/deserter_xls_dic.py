...
PATTERN_RTZK: Final = (
    r"(?:Призваний[:,]?\s*)?(?:(?:\d{2}\.\d{2}\.(?:\d{4}|\d{2}))\s*(?:року|[,.:;\s*])?\s*)?((?:\d+(?:\s?-\s?[а-яіїє]*)?\s?відділ(?:ом)?\s*)?[А-ЯҐЄІЇ][а-яґєії\'ʼ’]+(?:[-\s][А-ЯҐЄІЇ][а-яґєії\'ʼ’]+)*\s+(?:[ОМР]{0,2}[ТЦК]{1,3}|РВК|ОМВК|ОВК|РЦТК)(?:\s+та\s+СП)?(?:[\s,\.]*(?:(?:в\s+)?(?:м\.?|місто|н\.п\.?)\s*)?[А-ЯҐЄІЇ][а-яґєії\'ʼ’]+(?:[-\s][А-ЯҐЄІЇ][а-яґєії\'ʼ’]+)*)*(?:[\s,]+(?:област[ьі]|обл))?)"
)
