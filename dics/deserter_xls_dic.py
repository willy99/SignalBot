...
PATTERN_SUBUNIT_MAPPING : Final[Dict[str, str]] = {
    ...
    r'\зенітного\sракетного\sдивізіону\b': 'ЗРДн',
    r'\перехоплювачів безпілотних літальних апаратів\b': 'ЗРДн', # 'БатПБПЛА',
...
PATTERN_DESERTION_PLACE_MAPPING : Final[Dict[str, str]] = {
    ...
    r'(?i)після\s+(проходження|огляду)\s+(військово-лікарської\s+комісії|влк)': 'ВЛК',
...
PATTERN_RTZK: Final = r'(?i)((?:[А-ЯҐЄІЇ-][^.,!?\s]*\s+){1,5}?(?:[А-ЯҐЄІЇ]{0,2}(?:ТЦК|ВРК))(?:\s*(?:та|&)?\s*СП)?(?:[\s,]+(?:в\s+)?(?:м\.|місто)\s+[\w\-\']+(?:\s+[\w\-\']+)*)?(?:[\s,]+[\w\-\']+\s+(?:області|обл\.?))?)'
