...
from gui.tools.ui_components import fix_date, date_input, mark_clean, mark_dirty
...
def render_support_standard_page(controller: SupportController, person_controller: PersonController,
    ...
    async def on_save_draft_click():
        ...
        try:
            ...
            draft_new_id = await run.io_bound(
                ...
                package_type=DOC_PACKAGE_STANDART # Передаємо тип пакету
            )
    ...
    # ==========================================
    # ШАПКА ФОРМИ ТА КНОПКИ ДІЙ
    # ==========================================
    with ui.row().classes('w-full px-4 items-center justify-between mb-6'):
        ui.label('Стандартний супровідний лист (Зведений)').classes('text-3xl font-bold')
    ...
    with ui.grid(columns=12).classes('w-full px-4 gap-6 items-start'):
        ...
        with ui.card().classes('col-span-12 md:col-span-8 w-full'):
            ...
            # 💡 Спрощена форма вводу (тільки загальна кількість аркушів)
            ui.label('Інформація для додатку:').classes('mt-4 text-gray-500 font-bold')
            with ui.row().classes('w-full gap-4 mt-2 items-center bg-blue-50 p-4 rounded-md border border-blue-100'):
                total_input = ui.number('Загальна кількість аркушів', value=0, format='%.0f').classes('w-64 bg-white')
            ...
            def on_add_click():
                ...
                if total_val <= 0:
                    ui.notify('Вкажіть кількість аркушів у матеріалах!', type='warning')
                    return
            ...
            with ui.row().classes('w-full mt-4 gap-2'):
                add_btn = ui.button('Додати до списку', on_click=on_add_click, icon='add').classes(
