...
def render_home_page(auth_manager):
    ...
    with ui.column().classes('w-full items-center px-4 py-8 bg-gray-50 min-h-screen'):
        ...
        with ui.grid().classes(grid_classes):
            # --- АНАЛІТИКА ---
            if auth_manager.has_access('report_units', 'read'):
            ...
            if auth_manager.has_access('report_general', 'read'):
        ...
        with ui.grid().classes(grid_classes):
            ...
            if auth_manager.has_access('admin_panel', 'read'):
                ...
                # 💡 Наша нова сторінка
                create_nav_card('Налаштування', 'Глобальні параметри системи', 'settings', '/settings', 'slate')