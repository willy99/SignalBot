            ...
            with ui.tab_panel(main_tab):
                ...
                with ui.row().classes('w-full'):
                    ui.select(
                        ...
                        label=COLUMN_TITLE,
                        with_input=True  # Дозволяє фільтрувати список текстом
                ...
                with ui.row().classes('w-full'):
                    with ui.input(label=COLUMN_BIRTHDAY) as date_input:
                        date_input.bind_value(person, 'birthday')  # Прив'язуємо до моделі
                        with date_input.add_slot('append'):
                            ui.icon('edit_calendar').on('click', lambda: menu.open()).classes('cursor-pointer')
                        with ui.menu() as menu:
                            ui.date().bind_value(person, 'birthday')
                ...
                with ui.row().classes('w-full'):
                    ...
                    with ui.input(label=COLUMN_ENLISTMENT_DATE) as date_input:
                        ...
                        with date_input.add_slot('append'):
                            ui.icon('edit_calendar').on('click', lambda: menu.open()).classes('cursor-pointer')
                        with ui.menu() as menu:
                            ui.date().bind_value(person, 'enlistment_date')
                ...
                with ui.row().classes('w-full'):
                    with ui.input(label=COLUMN_DESERTION_DATE).classes('w-1/3') as date_input:
                        ...
                        with date_input.add_slot('append'):
                            ui.icon('edit_calendar').on('click', lambda: menu.open()).classes('cursor-pointer')
                        with ui.menu() as menu:
                            ui.date().bind_value(person, 'desertion_date')
                with ui.row().classes('w-full'):
                ...
                with ui.row().classes('w-full'):
                    with ui.input(label=COLUMN_RETURN_DATE) as date_input:
                        ...
                        with date_input.add_slot('append'):
                            ui.icon('edit_calendar').on('click', lambda: menu.open()).classes('cursor-pointer')
                        with ui.menu() as menu:
                            ui.date().bind_value(person, 'return_date')
                    with ui.input(label=COLUMN_RETURN_TO_RESERVE_DATE) as date_input:
                        ...
                        with date_input.add_slot('append'):
                            ui.icon('edit_calendar').on('click', lambda: menu.open()).classes('cursor-pointer')
                        with ui.menu() as menu:
                            ui.date().bind_value(person, 'return_reserve_date')
                with ui.row().classes('w-full'):
