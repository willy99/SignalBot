...
def test_402_refusal(processor_factory, mock_logger):
    ...
    assert person[COLUMN_CC_ARTICLE] == ARTICLE_402_REBELLIOUS
...
def test_desertion_locality_extraction(processor_factory, mock_logger):
    ...
    text = "Пошук військовослужбовця в районі зосередження підрозділу в н.п. Олександрівка Близнюківської селищної громади Харківської області, позитивного результату не приніс. Місце знаходження військовослужбовця невідоме."
    res = extract_locality(text)
    assert res == 'Олександрівка'

    text = "переданий в підпорядкування командира 3 батальйону морської піхоти військової частини А4765 самовільно залишив нове місце розташування підрозділу поблизу населеного пункту Шахтарське Синельниківського району Дніпропетровської області з особистою "
    res = extract_locality(text)
    assert res == 'Шахтарське'
...
def test_where_desertion_extraction(processor_factory, mock_logger):
    ...
    text = "25.04.2026 року близько 19:30 години, під час перевірки наявності особового складу був відсутній солдат ЗАЛУЖНИЙ Євген Романович, який не прибув з лікувального закладу до району виконання завдання за призначенням. Пошук військовослужбовця в районі зосередження підрозділом в н.п. Троїцьке Дніпропетровської області позитивного результату не приніс. Місцезнаходження військовослужбовця невідоме. Решта обставин з'ясовується."
    file_name = '23.04.2026 СЗЧ неповернення до РВБЗ (ЗАЛУЖНИЙ М.І.) рбс 3 дшб(4).doc'
    res = extract_desertion_place(text, file_name)
    assert res == 'лікування'
...
def test_desertion_type_extraction(processor_factory, mock_logger):
    ...
    cc = extract_cc_article(res)
    assert cc == ARTICLE_407_ABANDONEMENT
